...
export function gameInfoEmbed(
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Players', value: game.playerCap ? `${memberCount} / ${game.playerCap}` : `${memberCount}`, inline: true }
    );
  ...
  if (game.discordThreadId) {
    embed.addFields({ name: 'Thread', value: `<#${game.discordThreadId}>`, inline: true });
  }
