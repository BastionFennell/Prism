...
export const resumeSubcommand = new SlashCommandSubcommandBuilder()
  .setName('resume')
  .setDescription('Resume a paused game (Founder or GM only)')
  .addIntegerOption((o) =>
...
export async function handleResume(
  ...
  try {
    ...
    if (!canManageGame(interaction.member!, game, config)) {
      throw new AppError('Only Founders or the game GM can resume a game.');
    }

    await gameService.setStatus(gameId, 'active', interaction.user.id, config);
    await interaction.editReply({ content: `▶️ **${game.title}** has been resumed.` });
