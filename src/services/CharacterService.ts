...
import { DB } from '../db';
import { characterEntries, games, gameMemberships, CharacterEntry } from '../db/schema';
...
export class CharacterService {
  ...
  addCharacter(
    ...
    if (['archived', 'cleared'].includes(game.status)) {
      throw new AppError('Cannot add a character to an archived or cleared game.');
    }
    ...
    if (!isMember) throw new AppError('You must be a member of this game to add a character.');
  ...
  editCharacter(
    ...
    const canEdit = character.userId === actorUserId || isFounder || actorUserId === gmUserId;
  ...
  removeCharacter(
    ...
    const canRemove = character.userId === actorUserId || isFounder || actorUserId === gmUserId;
